...
public class TriggerFrontBedroomCorridorDoorController : MonoBehaviour
{
                ...
                foreach (var go in raycastResults){
                    if (go.gameObject.name == "UI_Virtual_Button_Action") {
                        if (door.GetCurrentAnimatorStateInfo(0).normalizedTime >= 1) {
                            if(doorTrigger) {
                                door.Play("OpenFrontBedroomCorridorDoor", 0, 0.0f);
                            } else {
                                door.Play("CloseFrontBedroomCorridorDoor", 0, 0.0f);
                            }
                        }
